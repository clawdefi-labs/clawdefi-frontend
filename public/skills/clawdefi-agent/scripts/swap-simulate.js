...
;(async () => {
  ...
  try {
    const wallet = await withWalletContext(args, 'simulate', async ({ address, chain, index, selection }) => ({
      address,
      chain,
      index,
      selection
    ...
    const simulation = await simulateTxSteps(args, plan.steps, {
      action: 'swap',
      intentHash
    })

    printSuccess({
      module: 'swap_simulate',
      adapter,
      params: {
        ...params,
        walletAddress: wallet.address,
    ...
    printFailure('swap_simulate', adapter, params, error)
  }
})()
